...
function App() {
        ...
        <div className="w-full">
          <div className="flex justify-between items-center mb-4">
            <div className="flex bg-white shadow-none border-none">
              <div 
                className={`relative text-gray-700 hover:text-gray-900 px-4 py-2 cursor-pointer bg-transparent border-none shadow-none ${activeTab === "assigned" ? "activeColorBG" : ""}`}
              ...
              <div 
                className={`relative text-gray-700 hover:text-gray-900 px-4 py-2 cursor-pointer ${activeTab === "unassigned" ? "activeColorBG" : ""}`}
            ...
            <Button className="buttonBGG hover:bg-green-700" onClick={() => setIsDialogOpen(true)}>
              ...
              <table className="w-full">
                <thead>
                  <tr className="tableBG">
                    <th className="py-3 px-4 text-left text-[12px] font-medium text-gray-500">TASK</th>
                    <th className="py-3 px-4 text-left text-[12px] font-medium text-gray-500">ID</th>
                    <th className="py-3 px-4  text-left text-[12px] font-medium text-gray-500">STATUS</th>
                    <th className="py-3 px-4 text-left text-[12px] font-medium text-gray-500">TIMER</th>
                    <th className="py-3 px-4 text-left text-[12px] font-medium text-gray-500">ASSIGNED TO</th>
                  </tr>
                </thead>
                <tbody>
  {tasks.map((task, index) => (
    ...
    >
      ...
      <td className="py-3 px-4">
        <div>
          <div className="text-[14px] font-medium">{task.assignedTo.name}</div>
          <div className="text-[12px] text-gray-500">{task.assignedTo.role}</div>
        </div>
      </td>
    </tr>
  ))}
      ...
      {isDialogOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm z-50">
         <div className="bg-white p-6 rounded-[20px] w-[90%] sm:max-w-[500px]">
